...
public:
    TLS_Client() : Command("tls_client host --port=443 --print-certs --policy= "
                           ...
                           "--test_main_dir= --test_case= --result_dir= "
...
# endif
      ...
      while(!client.is_closed())
      {
        ...
        {
          uint8_t req [] = "GET index.html HTTP/1.1\nHost: 127.0.0.1";
          ...
          std::cout << "got " << got << " bytes from read\n";
          if(got == 0)
          {
        ...
        if(FD_ISSET(m_sockfd, &readfds))
        {
          ...
          catch(std::exception const& e)
          {
            ...
            throw Botan::Invalid_State(
                    std::string(
                      "had an exception during last call to client.received_data(): "
                    ) + e.what()
            );
          }
        }
